...
class Project:
    ...
    @_closed_project_decorator
    def rename_mask(self, mask_id: int, new_name: str, experiment: str = 'default') -> None:
        """
        Renames a mask in the experiment.

        :param mask_id: the id of the mask
        :param new_name: the name to rename the mask to
        :param experiment: the name of the experiment to rename the mask
        """
        if mask_id < 0 or mask_id > 65535:
            raise ValueError("Mask ids must be between 1-65535")
        
        self.impl.RenameMask(experiment, new_name, mask_id)
    
    @_closed_project_decorator
    def get_mask_names(
        self,
        mask_ids: Iterable[int],
        experiment: str = "default"
    ) -> List[str]:
        """
        Gets a list of mask names associated with the provided ids.

        :param mask_ids: list of mask ids to retrieve the names of
        :param experiment: the name of the experiment where the mask is
        :return: an list of mask names with the same order as mask_ids
        """
        idList = pyglass.UShortList()
        for mask_id in mask_ids:
            if mask_id > 0 and mask_id <= 65535:
                idList.push_back(mask_id)
            else:
                raise ValueError("Mask ids must be between 1-65535")
            
        pyglassMap = self.impl.GetMaskNamesByIDs(experiment, idList)

        name_list = []
        for name in pyglassMap:
            name_list.append(name)
        
        return name_list
    
    @_closed_project_decorator
    def reset_mask_names(
        self,
        mask_ids: Iterable[int],
        experiment: str = "default"
    ) -> None:
        """
        Resets the mask names of a list of mask ids to their mask ids.

        :param mask_ids: list of mask ids to reset the name of
        :param experiment: the name of the experiment to reset the mask names
        """
        idList = pyglass.UShortList()
        for mask_id in mask_ids:
            if mask_id > 0 and mask_id <= 65535:
        ...
        pyglassMap = self.impl.ResetMaskNames(experiment, idList)
    ...
    def set_mask_color(
        ...
        """
        Sets the color of a mask.

        :param mask_id: the name of the mask to set the color of
        :param color: the color to set the mask represented as a 3-sized
                      tuple where the elements range from 0-255 and follow the
                      order r,g,b
        :param experiment: the name of the experiment to set the mask color
        """
        
        newColor = pyglass.ByteList()
        newColor.push_back(color[0])
        newColor.push_back(color[1])
        newColor.push_back(color[2])
        newColor.push_back(0)
        ...
        self.impl.UpdateMaskColorMap(experiment, idList, newColor)

    @_closed_project_decorator
    def set_mask_colors(
        self,
        mask_id_color_map: Mapping[int, Tuple[int, int, int]],
        experiment: str = "default"
    ) -> None:
        """
        Sets the colors of a list masks.

        :param mask_id_color_map:   dict that maps mask ids to the desired color.
                                    Color is represented as a 3-sized tuple where 
                                    the elements range from 0-255 and follow the
                                    order r,g,b
        :param experiment: the name of the experiment to set the mask colors
        """
        newColors = pyglass.ByteList()
        idList = pyglass.UShortList()
        for mask_id in mask_id_color_map:
            if mask_id > 0 and mask_id <= 65535:
                idList.push_back(mask_id)
            else:
                raise ValueError("Mask ids must be between 1-65535")
            
            color = mask_id_color_map[mask_id]
            newColors.push_back(color[0])
            newColors.push_back(color[1])
            newColors.push_back(color[2])
            newColors.push_back(0)

        self.impl.UpdateMaskColorMap(experiment, idList, newColors)
    ...
    def get_mask_colors(
        ...
        """
        Gets the colors of a list of masks.

        :param mask_ids: list of mask ids to retrieve colors for
        :param experiment: the name of the experiment where the mask is
        :return: a dict of mask names to rgb color values
        """
        idList = pyglass.UShortList()
        for mask_id in mask_ids:
            if mask_id > 0 and mask_id <= 65535:
        ...
        pyglassMap = self.impl.GetMaskIdColorMap(experiment, idList).asdict()
        
        mask_colors = {}
        for id in pyglassMap:
            color_abrg = pyglassMap[id]
            color_r = (color_abrg & 0x000000ff)
            color_g = (color_abrg & 0x0000ff00) >> 8
            color_b = (color_abrg & 0x00ff0000) >> 16
            mask_colors[id] = (color_r, color_g, color_b)
        return mask_colors
